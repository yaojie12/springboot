...
class Empress {
	...
	public void find(int i) { // 寻找皇后节点
		...
		for (int m = 0; m < 8; m++) { // 深度优先,递归算法     i是行  m是列
			if (rule(arry, i, m)) {    // 0  0
				arry[i][m] = 1;
				find(i + 1);
				arry[i][m] = 0;
			}
	...
	public boolean rule(int arry[][], int k, int j) { // 判断节点是否合适  0 0  1 0  1 1  1 2
		for (int i = 0; i < 8; i++) { // 行列冲突
			if (arry[i][j] == 1)
				return false;
		}
		for (int i = k - 1, m = j - 1; i >= 0 && m >= 0; i--, m--) { // 左对角线 -1 -1
			if (arry[i][m] == 1)
				return false;
		}
		for (int i = k - 1, m = j + 1; i >= 0 && m <= 7; i--, m++) { // 右对角线 0 1
			if (arry[i][m] == 1)
				return false;
		}
		return true;
